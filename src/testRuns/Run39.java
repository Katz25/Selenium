...
public class Run39 
    ...
    public static void main(String[] args) throws Exception 
		...
		// Should check how many clicks are available here
		...
		try
			...
			Envelope.addNamespaceDeclaration("soap", "http://hopewiser/soapbankcoder");									// Setup the Name space of soap
				QName Prefix1 = new QName("soapenv");																	// Setup the Name space of soapenv
				SOAPHeaderElement HeaderEl = Header.addHeaderElement(Prefix1);											// Initiate <Header> line into the message
					Prefix1 = new QName("oas");																			// Setup the Name space of oas
					SOAPElement Security = HeaderEl.addChildElement(Prefix1);											// Add a Child Element to the Header section called <Security>
						SOAPElement UsernameToken = Security.addChildElement(Prefix1);									// Add a Child Element to the <Security> section called <UsernameToken>
							// Enter Username
				...
				Prefix1 = new QName("soapenv");																			// Setup the Name space of soapenv
				SOAPBodyElement BodyEl = Body.addBodyElement(Prefix1);													// Initiate <Header> line into the message
				Prefix1 = new QName("soap");																			// Setup the Name space of soap
					SOAPElement ValidateReturnDetailRequest = BodyEl.addChildElement(Prefix1);							// Add a Child Element to the Header section called <Security>
		...
		URL endpoint = new URL("http://hopewiser/soapbankcoder");														// Set up endpoint variable to state where the message is sent to.
			...
			{
			// Print the message that has been built if there is an exception
			e.printStackTrace();
			}
